...
from scripts.deploy_chutes_runtime import explicit_metadata, miner_command
...
def test_deploy_wrapper_miner_command_contains_commit_metadata():
    ...
    assert "--subtensor.network" in command
    assert "test" in command
    assert "--netuid" in command
    assert "298" in command
...
def test_image_build_payload_uses_chutes_image_object():
    ...
    data, files = image_build_form_payload(image, public=True)
...
""",
    ...
    payload = chute_deploy_payload(chute_ref="sample_chute:chute", public=True)
...
def test_deploy_wrapper_build_deploy_flow_invokes_chutes_api_boundaries(
    ...
    monkeypatch.setattr(
        ...
        lambda *args, **kwargs: RuntimeMetadata(
            chute_id="chute-123",
            chute_slug="chronoseek-runtime",
    ...
    assert "--chute-slug chronoseek-runtime" in output
