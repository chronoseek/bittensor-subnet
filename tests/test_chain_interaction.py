    ...
    def test_miner_serve_axon(
        ...
        with patch.object(sys, "argv", test_args):
            ...
            mock_subtensor.serve_axon.assert_called_with(
                netuid=298, axon=mock_axon_instance
            )

            # 6. Check Uvicorn start
            mock_uvicorn.assert_called_with(
                unittest.mock.ANY, host="0.0.0.0", port=8091  # The FastAPI app
            )

            print(
                "✅ Miner Chain Interaction Test Passed: serve_axon called correctly."
            )
